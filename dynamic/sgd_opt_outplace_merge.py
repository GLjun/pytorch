...
class SGDOPO(Optimizer):
    ...
    def __init__(self, model, lr=0.1, momentum=0, dampening=0,
        ...
        super(SGDOPO, self).__init__(model.parameters(), defaults)
        ...
        self.prefix = "opt"
        self.cnt = 0
        print("SGDOPO alpha %f", self.alpha)

        #register buffer and hook
        self.register_buffer_and_hook()

    def __setstate__(self, state):
        super(SGDOPO, self).__setstate__(state)
    ...
    def register_buffer_and_hook(self):
        ...
        def wait_update_reduce_hook(m, gi, go):
            ...
            for key, param in m.named_parameters():
                if param.data is not None and param.requires_grad:
                    self.param_to_handle[param] = dist.all_reduce(m._buffers[self.prefix+key], dist.ReduceOp.SUM, async_op=True)
