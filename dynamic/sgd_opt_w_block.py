...
class SGDOPO_BW(Optimizer):
    ...
    def __init__(self, model, gpu=-1, lr=0.1, momentum=0, dampening=0,
        ...
        super(SGDOPO_BW, self).__init__(model.parameters(), defaults)
        ...
        self.model = model
        self.prefix = "opt_"
        self.all_data = "opt_all_data"
        self.cnt = 0
        self.gpu = gpu
        print("SGDOPO_BW alpha %f", self.alpha)
    ...
    def __setstate__(self, state):
        super(SGDOPO_MW, self).__setstate__(state)
    ...
    def register_buffer_and_hook(self):
        ...
        def gather_all_module_one_level(model):
            has_child = False
        ...
        #gather_all_module_one_level(self.model)
        module_list.append(self.model)

        def wait_update_reduce_hook(m, gi, go):
            # if there is a hook, there must be m in module_to_handle
            red = self.module_to_handle.get(m)
            if red is not None:
            ...
            for key, param in m.named_parameters():
                if param.data is not None and param.requires_grad:
                    ...
                    #    if not red.is_completed():
                    #        red.wait()
                    ave_param = m._buffers[self.prefix+key.replace('.', '')]
                    if self.alpha != 1.0:
            ...
            self.module_to_handle[m] = dist.all_reduce(m._buffers[self.all_data], dist.ReduceOp.SUM, async_op=True)
        ...
        for m in module_list:
            ...
            for param in m.parameters():
                if param.data is not None and param.requires_grad:
                    #if size_cnt==0:
                        #local_device = param.data.device
                    size_cnt += param.data.numel()
            
            if size_cnt == 0:
                continue
            
            #register all data in a module to a tensor
            all_data = torch.zeros(size_cnt, device='cuda:{}'.format(self.gpu), requires_grad=False)
            #print(" all data device ", all_data.device, " sel device ", self.gpu)
            m.register_buffer(self.all_data, all_data)

            offset = 0
            for key, param in m.named_parameters():
                if param.data is not None and param.requires_grad:
                    #buf = torch.tensor([], device='cuda:{}'.format(self.gpu))
                    #print(all_data.storage().device)
                    #buf.set_(all_data.storage(), offset, param.data.size()).requires_grad_(False)
                    buf = all_data[offset:offset + param.data.numel()]
                    buf = buf.view(param.data.size())
                    buf.copy_(param.data)
                    m.register_buffer(self.prefix+key.replace('.', ''), buf)
                    offset += param.data.numel()
            
            assert offset == size_cnt
