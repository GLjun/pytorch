...
class SGDAW(Optimizer):
    ...
    def __init__(self, params, lr=0.1, momentum=0, dampening=0,
        ...
        super(SGDAW, self).__init__(params, defaults)
        self.alpha = alpha
        self.cnt = 0
        print("SGDAW alpha %f", self.alpha)

    def __setstate__(self, state):
        super(SGDAW, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.alpha==1.0 or self.cnt < 102:
                    p.data.add_(-group['lr'], d_p)
                else:
                    p.data.mul_(self.alpha).add_(-group['lr'], d_p)
